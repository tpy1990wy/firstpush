...
public class PLSModifyOne {
	...
	PLSModifyOne(HDFSBalanceIndividual[] PL1,Vector<HDFSBalanceIndividual> PE1,Vector<HDFSBalanceIndividual> PP1,int PLSMax1,double[] idealPoint2) throws InterruptedException{
		int j=0;
		PLSMax=PLSMax1;
		...
		TPPLS H=new TPPLS(1);
			...
			for (int i = 0; i < PP1.size(); i++) {
				...
				NewInd.Var=PP1.get(i).Var.clone();
				int PNum=PP1.get(i).Var.length/H.Replication;
				Random R=new Random();
