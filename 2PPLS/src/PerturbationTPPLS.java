...
public class PerturbationTPPLS {
	int IID;
	...
	public PerturbationTPPLS(HDFSBalanceIndividual[] PL1,Vector<HDFSBalanceIndividual> PE1,Vector<HDFSBalanceIndividual> PP1,double[] idealPoint2) throws IOException {
		...
		for (int i = 0; i < PL1.length; i++) {
			...
			IID=0;
			PL1[i]=(HDFSBalanceIndividual) Pert.clone();
			...
			if (IID==1) {
				...
				if (IID==1) {
					...
					//更新参考点
						...
						if (PP1.get(PP1.size()-1).obj[0]<IdealPoint[0]) {
							IdealPoint[0]=PP1.get(PP1.size()-1).obj[0];
						}
						if (PP1.get(PP1.size()-1).obj[1]<IdealPoint[1]) {
							IdealPoint[1]=PP1.get(PP1.size()-1).obj[1];
						}
